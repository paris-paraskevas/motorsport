...
// Bet interaction for a GRID market: pick a driver AND their exact starting
// grid slot; settles on the QUALIFYING classification. DORMANT — deliberately
// not mounted anywhere yet (no automation opens grid markets and settlement has
// no quali source; see lib/betting/markets.ts createGridMarket). When it goes
// live, wire it in WeekendBetting beside ExactPositionBetCard. The odds map is
// keyed `driver@position` (position = grid slot), so this is the exact-position
// picker with grid wording. POSTs { marketId, pick: driver, position } — the
// server keys it as {driver, position}.
export function GridBetCard({
  market,
  balance,
  bets,
  leagues,
  ...
  const meta = MARKET_TYPE_META.grid;
  ...
  return (
    <div className="space-y-3 rounded border border-border p-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="font-mono text-xs text-text-muted">to qualify</span>
        <select
          value={position}
          onChange={e => setPosition(Number(e.target.value))}
          className="rounded border border-border bg-surface px-2 py-1.5 font-mono text-sm text-text"
          aria-label="Grid slot"
        >
          {positions.map(p => (
          ...
          {bets.map(b => {
            ...
            return (
              <li key={b.id} className="flex items-center justify-between py-1.5 font-mono text-sm">
                <span className="text-text">Your bet · {d} grid P{String(b.selection.position)} · {b.stake}</span>
