...
**Tech Stack:** Next.js 16 App Router (React 19), TypeScript, Tailwind CSS v4 (CSS-driven, no `tailwind.config.ts`), `@serwist/next` (PWA), `node-ical`, `gray-matter`, `remark` + `remark-html`, `@tailwindcss/typography`, `vitest`.

> Note: spec called for Next 15 + Tailwind v3 but `create-next-app@latest` installed Next 16 + Tailwind v4. User decided to adapt the plan rather than downgrade. `@serwist/next` replaces `@ducanh2912/next-pwa` because the latter has no Next 16 support.
...
AGENTS.md                                # create-next-app's Next-16 agent notes; keep
CLAUDE.md                                # just imports AGENTS.md; keep
postcss.config.mjs                       # Tailwind v4 PostCSS plugin (created by create-next-app)
vitest.config.ts
...
```bash
npm install node-ical gray-matter remark remark-html
```

- [ ] **Step 2: Install PWA (Serwist, Next-16 compatible)**
...
Note: `@types/node` is already a devDependency from create-next-app.

- [ ] **Step 5: Add npm scripts**

In `package.json`, ensure `"scripts"` contains (preserve the existing `dev`, `build`, `start`, `lint`):
...
git commit -m "feat: add runtime, PWA, typography, and test deps"
```
...
## Task 11: Root layout + global styles + Tailwind typography (Tailwind v4)

**Files:**
- Modify: `app/layout.tsx`
- Modify: `app/globals.css`

> Tailwind v4 is config-less and CSS-driven. The typography plugin is loaded via `@plugin` in CSS, not a JS config file. Dark mode is forced by overriding the variant and the body color directly (we do NOT use `prefers-color-scheme`).

- [ ] **Step 1: Replace globals.css with v4-style dark theme**
...
- [ ] **Step 3: Verify dev server renders dark page**

Run: `npm run dev` in the background, `curl -sI http://localhost:3000 | head -5` to confirm a 200 response, then kill the dev server.

- [ ] **Step 4: Commit**

```bash
git add app/layout.tsx app/globals.css
git commit -m "feat(layout): force dark theme + typography plugin (Tailwind v4)"
```
...
## Task 16: PWA configuration (Serwist)
...
> Why Serwist: `@ducanh2912/next-pwa` does not officially support Next 16. Serwist (`@serwist/next` + `serwist`) is the maintained successor used by the Next.js community for v15+/v16. It compiles a TypeScript service-worker source (`app/sw.ts`) to `public/sw.js` at build time.

- [ ] **Step 1: Wrap next.config.ts with Serwist**
...
- [ ] **Step 4: Generate placeholder PNG icons**
...
- [ ] **Step 5: Wire the manifest into the layout**

In `app/layout.tsx`, ensure the `metadata` export contains `manifest: '/manifest.json'`. (Done in Task 11 — verify still present.)

- [ ] **Step 6: Verify build succeeds**

Run: `npm run build`
Expected: build completes; `public/sw.js` generated by Serwist. No errors about `app/sw.ts`.

- [ ] **Step 7: Verify install affordance in Chrome (manual)**
...
```bash
git add next.config.ts app/sw.ts public/manifest.json public/icons/
git commit -m "feat(pwa): serwist service worker, manifest, placeholder icons"
```
