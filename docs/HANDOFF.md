...
### ▶▶ SESSION CONTINUATION 2026-07-02 (later) — 0.152.0 in PR #364 (UNMERGED), prod-verify GREEN, MotoGP chart gated

Continued after the merge. Operator asked to work items 1–9 of the open-list. Status:

- **✅ PROD VERIFY (was owed) — all green.** Prod is live at **0.151.0**. **`/series/nls/results` on the datacenter returns the winners** (Overall winners + Rowe Racing/Dunlop, no "temporarily unavailable") — the #360 NLS fix WORKS on prod, bug confirmed dead. Anon gate teaser confirmed live on a prod F1 quali session (Decoder markers absent). No action needed.
- **✅ PR #364 (0.152.0) — search type-filter + local-dev resilience — OPEN, browser-verified, NEEDS YOUR MERGE** (classifier blocks agent self-merge of a fresh PR). Contents: (a) search overlay **type-filter chips** (All/Drivers/Teams/Series/Weekends/Series-tabs/Blog/Pages, live counts, active chip narrows results, new query resets to All) — verified live (Weekends→8, 0 console errors); (b) **`lib/userPrefs.ts` KV writes degrade to no-op in DEV** (prod still throws — real-misconfig guardrail) so a store-less local build stops 500-ing; (c) **`OnboardingWizard` device-local `paddock:onboarded` flag** so the wizard can't loop when the server can't persist; (d) fixed the 0.149.0 RELEASES ⌘K (Mac-only) copy. This also resolves the "local 500 / wizard re-shows" note.
- **🔴 MotoGP chart bug — DIAGNOSED + GATED in #364.** Root cause: the trend chart derives from the per-session **results feed** (motogp.com results API) while the standings **table** is the authoritative motogp.com standings — two different sources that genuinely diverge (feed under-counts Bezzecchi −13 / Ogura −8 / Di Giannantonio −25; chart carries 28 rider series vs 27 standings rows). NOT a finisher-floor (already lowered) and NOT a fixable-here name split. Per the **locked chart==standings invariant**, the fix is to **drop the chart until it reconciles** (like WRC/GTWCE/IMSA/FE) — done in `components/tabs/StandingsTab.tsx` (MotoGP branch, ~L711). **Deeper follow-up (IDEAS):** reconcile the two upstreams round-by-round for the 3 riders → curate `content/series/motogp/results-overrides.json` for the missing rounds (or find the feed gap), then restore the chart.

**▶ STILL OPEN for next session (operator "do 1–9", items not yet done):**
- **#11 — get main green:** `lib/openf1/turns.test.ts` ×2 deterministic failures (real turn-detection edge bug vs stale test — investigate `detectTurns` minSpacing/numbering) + `lib/sitemap-data.test.ts` flaky (date/async-dependent; the fixture assumes an earlier date — many 2026 rounds are now past). NOT started.
- **#14 — Arc-2 gating scope + build (DECISION-GATED):** operator earlier said wall social/notifications/home-customize/following. `/social` + notifications are already API-gated; the genuinely-new + high-blast-radius part is walling **following + home-customize** = reversing the device-local-guest model (anon follows/customizes via localStorage today). Needs the explicit go on that reversal, then build (PR, hold merge for visual). Doc: `docs/research/2026-07-02-account-gating.md`.
- **#15 — Arc-2 IA increment-1 build:** the doc's rec = Home v3 spine (Up-next → Just-missed, above the customizable zone) + a "Jump to" launcher (Calendar/Analysis/Standings▾/Results▾/News, with a series-picker popover) + a weekend/session breadcrumb. High-blast-radius on Home/AppShell → build as a PR for your visual review. Doc: `docs/research/2026-07-02-ia-restructure.md` (+ 7 taste Qs).

**Merge order when ready:** #364 is off `main`, clean (no stack) → merge anytime (operator-initiated). gh on `paris-paraskevas`. Dev server parked on `feat/search-type-filter`.
