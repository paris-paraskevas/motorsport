...
### 📋 Next session — operator's stated order
1. **Cloudflare Git auto-deploy** (Workers Builds) so pushes deploy.
2. **Add the `dev.*` admin route** (dev.paddock-tracker.com → the Worker).
3. Then reassess remaining tasks.
