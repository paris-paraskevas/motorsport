...
## ⚡ Next session pickup — 2026-08-05 (LATEST, session 25 — calendar rebuild + learn hub on `testing`; TWO OPEN DECISIONS) — `testing` = 3628422, NOT merged to `main`

Everything below is on **`testing` only**, live at `testing.paddock-tracker.com`. `main` and prod untouched. Version stays `0.244.1` — testing-branch commits don't bump (session-24 precedent: its `feat(home)` commits touched no version files).

### 🛑 RESUME HERE — two decisions, both operator-owned
1. **Learn-page imagery source.** A senior designer's review asked for background photos on the 10 topic cards and series logos above each guide. **Logos are refused outright** — killed 2026-07-12 (`IDEAS.md:150`), no free/non-infringing source, championship logos are trademarks. Driver portraits ×14 killed the same day. Research done this session (scratchpad only, nothing written): a Commons API sweep filtered to CC0/CC BY/CC BY-SA/PD, ≥1000px, HEAD-checked live returned **97 usable candidates across the 10 topics** — strong for endurance (`Le Mans 2013 (63 of 631).jpg`, CC BY 2.0, 4608×3456), tracks (Zandvoort aerial, CC BY 2.0, 5464×3640), Motorsport 101 (Nürburgring pole markings, CC BY-SA 3.0) and stock cars. **Thin or empty for MotoGP, Teams, Drivers and rally-in-action** — and relevance can't be automated (free-text search offered a Lego store for NASCAR, a nationality map for Drivers; the parent categories `MotoGP motorcycles in action`, `Formula One pit stops`, `WRC cars in action` all return **0 files** because photos live in deep per-event subcategories). Two practical constraints: CC BY / CC BY-SA **require visible credit** (pattern exists — `portraits.json` + `app/(app)/drivers/[slug]/page.tsx:348`), and `next.config.ts:61` sets `images: { unoptimized: true }` for the Workers runtime, so multi-MB Commons originals must be **downscaled and self-hosted**, not hotlinked. **Recommendation: hybrid** — photos on the ~6 topics with a strong free candidate, owned generated art for the abstract ones (Teams, Drivers). ~2h. Operator should eyeball the 10 picks before they land.
2. **Series-colour palette collision.** `adac-ravenol-24h` and `nls` are the **identical hex `#fbbf24`** — two series, one colour, so the new legend cannot decode them. **11 of 105 pairs** fall below the OKLab 0.10 confusable threshold; the amber family is five series deep (MotoGP `#fb923c`, WorldSBK `#f59e0b`, WRC `#eab308`, NLS + ADAC `#fbbf24`), which is why Fri/Sat/Sun read as an orange wash. Also IndyCar `#f43f5e` vs F1 `#ff4136` at 0.050, Formula E `#06b6d4` vs F2 `#38bdf8` at 0.057. Fix = designed 15-colour categorical set across up to 15 `meta.json` files + a validator test asserting no pair under threshold; ripples to `/app`, series pages and standings charts. Needs operator sign-off on which colours may move (hold F1 red, MotoGP orange).

### ✅ Shipped to `testing` this session (7 commits, `fc8d134..3628422`)
- **`91abab3` calendar grid helpers** — `groupIntoEvents` (collapses a day into one block per series-round; 15 sessions across 6 series = 6 rows, which is what kills the `+12 more` cliff), `seriesCode` (explicit 15-series abbreviation map + derived fallback; a phone weekday column is ~38px where "GT World Challenge" clips to "GT W"), `sessionTimeLabel` / `tzLabel`, and an optional `utc` flag on `localDayKey` / `bucketByDay` that moves the **bucket** as well as the label. `SESSION_KINDS` makes `other` a first-class filter kind. 11 → 21 tests.
- **`a3d8783` /calendar rebuilt on the /app design system** — control deck (full-bleed UP NEXT band, month arrows against their label, Today restored, series legend doubling as the filter, local/UTC control), **Weekends agenda** built on `groupByWeekend`, weekend-weighted month columns (0.62fr vs 1fr), event chips instead of session pills, month footing. Fixed two defects: day cells were `role="button"` with nested `<Link>`s (42 tab stops → one `role="grid"` with roving tabindex + arrow keys), and `allTypes = types.size >= 3` silently hid every unclassified session whenever any named kind was deselected.
- **`bcd528a` month-grid fills** — self-inflicted by `a3d8783`; see landmine 1.
- **`4a0e1a5` up-next band to hero type** — title 18→44px, countdown 14→36px, band ~64→162px, two tiers like the /app chyron, venue added, title wraps (clamped to 2 lines) instead of truncating. Below `sm` the title takes `basis-full` or it was ~150px wide.
- **`75092d5` month default + state in the URL** — `?view=` and `?d=` (day precision; `?m=` stays a read-only inbound form for the header menu). localStorage holds the **view** so a fresh visit resumes it; the **anchor is deliberately not persisted**. Precedence URL > localStorage > month.
- **`32be179` replaceState, not router.replace** — see landmine 3.
- **`3628422` learn hub (`/information`) on the design system** — section heads **14px → 24-30px** (the page fell straight from a 48px `h1` to 14px), **zero → panels on every section**, HISTORY/RULES links **50×15px → ≥32px pills** (30 of them, under the 24×24 WCAG 2.2 SC 2.5.8 minimum; the topic pages' `All topics` back-link was 70×16), answers cardified in `EntryRow` so all 5 call sites benefit, topic counts read "142 answers", stray `rounded-lg`/`rounded-md` dropped, the bare "Explore every series" link over 80px of dead space became a panel. New shared `SectionHead` + `PillLink` in `InfoUi` (two consumers: hub + topic index). Tracks country labels 11 → 14px.

### ⚠ Session 25 landmines
1. **Text tokens pass AA; the BORDERS are what fail.** `--text` 18.48:1, `--text-muted` 7.85:1, `--text-faint` 5.43:1, `--brand` 11.29:1 on `--bg` — all clear AA, so "use brighter off-white" (asked twice in review) would flatten the documented text→muted→faint hierarchy for **no** gain. What fails is `--border` at **1.42:1** and `--border-strong` at **1.64:1**, against the 3:1 WCAG 2.2 asks of a boundary that carries meaning. **`--bg` → `--surface-elevated` spans only 1.164:1 in total**, so no arrangement of those fills can differentiate anything in near-black; they exist to lift a panel off a page, not 42 cells from each other. Structure in near-black must come from a rule at `--text-faint` weight (4.96:1 on the panel) or from type size. The real readability defects on both pages were **10-11px type** and **absent surface panels**.
2. **Element opacity on a container INVERTS its fill.** `bg-bg/60` + `opacity-60` on a month cell composited *lighter* than plain `bg-surface`, so adjacent months read as lifted rather than receded (measured: out-of-month `rgb(24,24,31)` vs in-month `rgb(20,20,26)`). Opacity fades the background toward the lighter backdrop. **Dim the contents, keep the container's fill opaque.**
3. **Never `router.replace` for pure query-state sync.** The App Router's client cache is keyed on the full URL, so changing `?view=` is a cache miss → RSC refetch → the subtree drops to its Suspense fallback. Since that fallback is `CalendarSkeleton`, **every view toggle flashed the skeleton**. Use `window.history.replaceState`; Next documents it as integrating with the router and keeping `useSearchParams` in sync — `node_modules/next/dist/docs/01-app/02-guides/single-page-applications.md`, "Shallow routing on the client". Back still restores exactly.
4. **HTML entities are NOT decoded inside a JSX attribute.** `title="Guides &amp; tracks"` renders the literal `&amp;`.
5. **A wedged dev server fakes app bugs.** Starting a second `next dev` while one runs produces an EPIPE flood and `500`s reading `Jest worker encountered 2 child process exceptions` — which looks exactly like a broken route. `/information/*` returned 500 for that reason and I wrongly blamed my own change; all four routes were 200 on a clean restart. Kill the old PID first.
6. **Playwright: a locator screenshot scrolls its element into view**, so any `boundingBox()` read *before* it is stale by the scroll offset. Screenshot first, then read every rect in one pass, relative to the element's own origin. Two other harness traps hit this session: fixed sleeps after a view click race the soft nav, and polling `page.evaluate` every 16ms from Node destabilises the navigation being measured (observe from inside the page with a `MutationObserver` instead). **Three separate times a failing check was the harness, not the app** — verify the app directly before believing a red result.

### ❌ Two review claims that did not survive checking
- **"Nav says KALENDAR with a K."** Zero hits for `KALENDAR`/`Kalendar` anywhere in the codebase; the rendered nav label read `Calendar` from the live DOM. Nothing to fix — likely a browser translation extension.
- **"Huge black void at the bottom of the learn page."** Measured 80px, of which 64px is `PAGE_WIDE`'s intentional `pb-16`. Real but modest; the defect was a bare link floating in it, now a panel.

### Verification standing at session close
`tsc` clean · `eslint` clean · **961 tests** · calendar four-view regression at 390/1280/1920 (no h-overflow, one grid tab stop, arrow keys, UTC toggle, legend filter, Today) · composited-pixel fill checks · 17 persistence checks (Back restores `?view=week&d=2026-07-28` exactly, forward/back stable, four toggles add **zero** history entries) · no-flash checks (zero skeleton mounts across five switches) · learn checks at three widths on `/information`, `/information/endurance`, `/information/tracks`. **One console error remains and is pre-existing and app-wide:** a `data-theme` hydration mismatch from the no-flash theme init, reproduced identically on `/app` and `/series/f1`.

---

## ⚡ Session 24 — 2026-07-29 (home magazine rebuild on `testing`; ONE BLOCKING DECISION) — `testing` was 9e957e4, NOT merged to `main`
