...
## ⚡ Next session pickup — 2026-06-22 (main = 0.49.0) — **Paddock Betting is LIVE (F1)**
...
4. **◑ New market types — PODIUM ENGINE DONE (0.49.0), dormant.** Podium (top-3) pricing (`podiumProbabilities` via Harville/Plackett-Luce, `podiumMultipliers`), `createPodiumMarket`, settlement (migration `20260622120000_podium_settlement.sql` + `settleLeagueMarket`/`settleDueMarkets` + new `podiumForRound`), tests, and `scripts/verify-podium.mts` (ran green vs local — in-podium pick paid, off-podium lost, 1W/1L). **Dormant — go-live:** (1) apply migration `20260622120000` to prod (`supabase db push`) + run `verify-podium.mts`; (2) call `createPodiumMarket` in `openUpcomingMarkets` AND wire the weekend UI to render **multiple markets per round** (`/api/bet/market` returns one market today → needs an array; `MarketBetCard` selection key becomes `driver` for podium). **Remaining types:** **top-10** = podium pattern with slots=10; **exact-position** needs a finishing-position distribution model; **grid/qualifying-position** needs a quali-pace model + a `market_type` enum addition (`alter type … add value`).
