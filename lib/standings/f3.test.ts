...
// `fetchF3Standings` can be exercised. `snapshotConfigured` defaults FALSE so
...
// flips it on. Mirrors lib/standings/dtm.test.ts.
...
// Durable last-good: a fiaformula3.com outage should serve the previous good
...
describe('fetchF3Standings — durable last-good (source_snapshot)', () => {
  ...
  it('SUCCESS persists the standings under standings:f3', async () => {
    mockFetch(FULL_DRIVER_FIXTURE, FULL_TEAM_FIXTURE);
    const data = await fetchF3Standings();
  ...
  it('FAILURE serves the last-good standings instead of null', async () => {
    mockFetch(FULL_DRIVER_FIXTURE, FULL_TEAM_FIXTURE);
    await fetchF3Standings(); // prime the snapshot
    mockFetch(FULL_DRIVER_FIXTURE, FULL_TEAM_FIXTURE, true, false); // team page 500s
    const recovered = await fetchF3Standings();
    expect(recovered).not.toBeNull();
    expect(recovered!.drivers).toHaveLength(14);
  ...
  it('a good fetch overwrites the snapshot (self-heal)', async () => {
    ...
    snapshotTable.set('standings:f3', {
      drivers: [{ position: 1, driverName: 'stale', team: 'x', points: 0, wins: 0 }],
      constructors: [],
    });
    mockFetch(FULL_DRIVER_FIXTURE, FULL_TEAM_FIXTURE);
    await fetchF3Standings();
    const stored = snapshotTable.get('standings:f3') as { drivers: unknown[] };
    expect(stored.drivers).toHaveLength(14);
  });
