...
- **PSI sweep across every major page** (operator, 2026-08-20: "we also need a session to PSI run on every major page") — one measurement session: PSI mobile+desktop per major template (`/`, `/app`, series landing, standings, results, weekend, session, driver, calendar, blog post, information guide), a per-page baseline table appended to `docs/perf-baselines.md`, then fix the worst offenders in ranked order. Needs either a PageSpeed API key (free, lets Claude script the ~20 runs) or the operator clicking pagespeed.web.dev per page.
