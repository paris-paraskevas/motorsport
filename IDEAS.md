...
- Generalised upstream-feed resilience (operator 2026-06-24, off the Jolpica outage): extend the F1 KV last-good pattern (0.84.0) to every flaky feed (standings/results per series) + a cache-warm cron so the site serves from cache and never hits a live endpoint on the request path. (DB-as-cache rejected for now — Supabase is eu-west-1 / compute iad1, so a Postgres read per render adds the transatlantic hop the region note already flags; KV is the right tier until compute moves to the EU.) Supersedes the older "standings last-good resilience" inbox item.
- **Forecast market went LIVE 0.88.0 (#234)** — operator decision (Now §1e); exact_position still held (Now §1d). Watch the first real forecast settlement after an F1 race; verify the picker UI signed-in.
