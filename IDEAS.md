...
- Betting — new market types (operator 2026-06-22): PODIUM engine shipped dormant 0.49.0 (Harville pricing + settle migration + verify; not auto-opened / UI-wired — go-live steps in HANDOFF); remaining = top-10 (podium pattern, slots=10), exact-position (needs a finishing-position distribution model), grid/qualifying-position (needs a quali-pace model + a `market_type` enum addition).
