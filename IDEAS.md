...
- Leagues overhaul (operator 2026-06-22, APPROVED + building): dedicated league page, per-league settings, per-member nicknames + colours (anyone can set for anyone), invite LINKS (not just code) that prompt account-create + join + a friend request to the inviter (prompted accept, not auto), month/season prizes = titles/badges for top 3. Needs a global friends graph (in-league you can friend non-friends). Phased: **P1 friends DONE 0.55.0** (`friendship` table + `/api/friends` + `/play` Friends section + verify), P2 invite links + join-&-befriend, P3 league page + nicknames/colours/settings, P4 prizes. Friends global; leagues invite-only.
