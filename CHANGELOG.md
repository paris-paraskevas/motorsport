...
### Changed
- **Session close — Sunday docs triage.** `IDEAS.md` Now / Next refreshed (Sunday's deliverables retired, Monday priorities promoted); 14 new Inbox items added covering today's discoveries (per-session results-fetch lifecycle as #76, ADAC single-event special-casing, head-to-head drivers + constructors, IA restructure, admin UI, app best-practices pass, perf pass, GDPR consent banner, Notification badge chequered-flag motif, contact form categories, calendar month-tab swipe, Live Now expansion, Champions visual redesign, notification sound refinement). `SCHEDULE.md` closes Sunday with all 9 shipped versions logged + sketches Monday (rounds.json bottom-8, IMSA P1 + FE Sanya + F1 Azerbaijan curation, weather+news coverage audit, ADAC special-casing) and Tuesday Fotis sit-down agenda. `docs/HANDOFF.md` infra ledger updated with the 9 shipped versions, "Next session priorities" rewritten for Monday, new "What shipped Sunday" section, Saturday task state honest-update.
