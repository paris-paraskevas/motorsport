-- Paddock Betting — podium (top-3) settlement.
-- Extends the solo-book settle_market RPC to settle 'podium' markets alongside
-- 'winner'. A podium bet's selection is {"driver":"<name>"}; it wins when that
-- driver is in the official top 3, passed as p_result = {"podium":["P1","P2","P3"]}.
-- Payout is stake × the per-driver multiplier stored on the market at creation
-- (fixed odds), exactly like winner. top10 / exact_position still mark lost
-- (future types). Provisional-is-final, settle-once — unchanged from the original.
--
-- Pure CREATE OR REPLACE (additive): the winner branch is byte-for-byte the prior
-- behaviour, so re-applying is safe and existing winner markets are unaffected.
...
begin
  ...
  for r in select * from bet where market_id = p_market_id and outcome = 'pending' loop
    if v_type = 'winner' then
      ...
      v_sel := null;
      v_win := false;  -- top10 / exact_position: future types
    end if;
