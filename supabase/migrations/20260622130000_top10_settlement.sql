-- Paddock Betting — top-10 (points finish) settlement.
-- Extends settle_market to settle 'top10' markets alongside 'winner' and 'podium'.
-- A top10 bet's selection is {"driver":"<name>"}; it wins when that driver is in
-- the official top 10, passed as p_result = {"top10":["P1",...,"P10"]}. Payout is
-- stake × the per-driver multiplier stored on the market at creation (fixed odds),
-- exactly like winner/podium. exact_position still marks lost (future type).
--
-- Pure CREATE OR REPLACE (additive): the winner + podium branches are unchanged
-- from the prior migration, so re-applying is safe and existing markets settle
-- identically.
...
begin
  ...
  for r in select * from bet where market_id = p_market_id and outcome = 'pending' loop
    if v_type = 'winner' then
      ...
      v_sel := null;
      v_win := false;  -- exact_position: future type
    end if;
