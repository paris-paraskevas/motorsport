...
**Internal housekeeping.** Retired unused code left over from an earlier landing-page design; nothing user-facing changed.
...
**Calendar tidy-up.** Upcoming race weekends now list in a single full-width column, easier to scan than the old two-column grid.
...
**Behind the scenes.** Our internal click-heatmap now shows a true visual overlay — the real page with colour hotspots landing exactly on the buttons and tabs people actually clicked.
...
Reverted yesterday's experimental 3D onboard scene — it wasn't right (walls everywhere, misplaced scenery, an odd-looking car). The onboard is back to the clean version while we rebuild the realistic scene properly.
...
**Weekend pages, streamlined.** Session links now sit under the Schedule tab (with standings one tap away), so there's one less tab. And "Bet on the weekend page" from Play drops you straight onto the Bets tab.
...
*Internal: documentation/handoff — no changes to the app.*
